...
//   const sendBlogRequestToBackend= ()=>{
...
console.log("I entered the blog page ");
...
function fetchPaginatedblogPostListFromBackend() {
  ...
  fetch(`${siaLicenseBaseUrl}/api/v1/sialicence+/blog/allBlogPosts`, {
      method: 'POST',
      headers: {
...
function sendBlogPostJsonDataToBackend() {
  ...
  fetch(`${siaLicenseBaseUrl}/api/v1/sialicence+/blog/blogPostCreation`, {
    method: 'POST',
    headers: {
        'Content-Type': 'application/json',
...
function display(listOfBlogPost){
  ...
  <div data-index=${index} class="d-flex gap-2 flex-wrap align-items-center">
      ...
      <div class="flex-fill">
          <a href="javascript:void(0);" class="fs-14 fw-semibold mb-0">${blogPost.blogPostCategory}</a>
...
function populateForm(selectedBlogPost) {
  ...
  //document.getElementById('publishStatusId').options[0].setAttribute('value', selectedBlogPost.publishStatus);
...
function deteleBlogPost(blogPostId){
  ...
  fetch(`${siaLicenseBaseUrl}/api/v1/temisplace/blogPostDeletion?id=${encodeURIComponent(blogPostId)}`, {
    method: 'DELETE',
    headers: {
        'Content-Type': 'application/json',
     //   "Authorization" : jwtToken
...
function updateBlogPostStatus(dropDownText , blogPostId){
  ...
  fetch(`${siaLicenseBaseUrl}/api/v1/temisplace/blogPostStatusUpdate`, {
    method: 'PATCH',
    headers: {
        'Content-Type': 'application/json',
       // "Authorization" : jwtToken
